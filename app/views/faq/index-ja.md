...
* `Sender` と `yourdomain.com` ごとに1分あたり50通のメッセージのバースト制限があります。これにより、日次制限に達していない場合でも、スパマーが1秒あたり数百通のメッセージでドメインを氾濫させることを防ぎます。

すべてのレート制限はアトミックに適用されます — カウンターはメッセージが保存される前にインクリメントされ、同時リクエストが制限を回避できる競合状態を排除します。
