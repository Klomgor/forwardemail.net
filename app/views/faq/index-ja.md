...
* [システムおよび技術的詳細](#system-and-technical-details)
  ...
  * [帯域幅制限はどのようになっていますか](#what-are-your-bandwidth-limits)
...
7. メッセージに `Feedback-Type` ヘッダー（[RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965) に基づく ARF 不正利用フィードバックレポートを示す）が含まれている場合、送信しません。

8. MAIL FROM アドレスが ARF フィードバック送信者のパターン（例: `feedback@arf.mail.yahoo.com`）と一致する場合、送信しません。
...
* 有料プラン（強化保護およびチーム）では、エイリアスごとにWKD検索を無効にできます。エイリアス設定に移動し、「WKD検索を無効にする」オプションをチェックしてください。無効にすると、そのエイリアスに転送される受信メールは、受信者の公開鍵が公開されていてもWKDによる暗号化が行われません。これはAPIでも`has_wkd_disabled`フィールドを使用して利用できます。
...
* 個々のエイリアス単位（例：`you@yourdomain.com`）で受信メールをレート制限する代わりに、エイリアスのドメイン名自体（例：`yourdomain.com`）でレート制限を行います。これにより、`Senders` がドメイン内のすべてのエイリアスの受信箱を一度に氾濫させるのを防ぎます。
* レート制限は、送信者の信頼レベルに基づく階層型システムを使用して適用されます：
  * **Tier 1 – 真実の情報源**（例：`gmail.com`、`microsoft.com`、`apple.com`）：グローバルで1日あたり100 GBに制限されます。ドメインごとの制限およびバースト制限は免除されます。
  * **Tier 2 – [許可リスト](#do-you-have-an-allowlist)に登録されている送信者**：グローバルで1日あたり10 GBに制限されます。ドメインごとの制限およびバースト制限は免除されます。
  * **Tier 3 – その他すべての送信者**：グローバルで1日あたり1 GBおよび/または1000通のメッセージ、`Sender`+ドメインごとに1日あたり1 GBおよび/または1000通のメッセージ、さらに `Sender`+ドメインごとに1分あたり50通のメッセージのバースト制限が適用されます。
* バースト制限は固定ウィンドウカウンター（60秒）を使用します。ウィンドウは最初のメッセージが到着したときに開始され、後続のメッセージに関係なく60秒後に期限切れになります — メッセージごとにスライドしたりリセットされたりすることはありません。
* 受信者のメールボックスごとに1日あたり100,000通のメッセージの上限があります。これはすべての階層に適用され、送信者の信頼レベルに関係なく、単一のメールボックスが氾濫するのを防ぎます。

すべてのレート制限はアトミックに適用されます — カウンターはメッセージが保存される前にインクリメントされ、同時リクエストが制限を回避できる競合状態を排除します。デクリメント操作（インクリメント後にストレージが失敗した場合に使用）は、カウンターが負になるのを防ぐ安全なLuaスクリプトを使用します。

MXサーバーは、1つ以上の受信者への転送メッセージもレート制限により制限しますが、これは[許可リスト](#do-you-have-an-allowlist)にない `Senders` のみが対象です：
...
* レート制限を超えた `Senders` は421エラーで拒否されます。
...
すべてのサービスにおいてユーザーごとの帯域幅制限を適用し、フラッディング攻撃を防止しつつ、正当な使用に対して十分に寛大であり続けます。これらの制限は意図的にGmailの制限をはるかに上回っています — 大容量のバックアップをインポートしたり、メールボックス全体を新しいデバイスに同期したり、複数のクライアントを同時に使用しても、制限に達することはありません。

| 制限 | 範囲 | 量 |
| :--- | :--- | :---: |
| 日次合計 | 全サービス合算（IMAP、POP3、SMTP、CalDAV、CardDAV） | **50 GB** |
| サービスごとの時間制限 | 個別サービスごと（例：IMAPダウンロード、SMTPアップロード） | **10 GB** |

日次制限はすべてのプロトコルで共有される単一の予算です — IMAPでダウンロードしても、SMTPでアップロードしても、CalDAVでカレンダーを同期しても、すべて同じ50 GB/日にカウントされます。サービスごとの時間制限は、単一プロトコル上の暴走スクリプトや侵害されたアカウントに対するセーフティネットであり、正当なユーザーが到達すべきものではありません。

これらの制限はユーザーアカウントごとに適用され（エイリアスやドメインごとではありません）、毎日リセットされます。つまり、追加のエイリアスを作成しても帯域幅の割り当ては増加しません。Redisが利用できない場合、レート制限は完全にスキップされ（フェイルオープン）、サービスが中断されることはありません。

特定のユースケース（例：非常に大きなアーカイブの移行）でより高い制限が必要な場合は、[お問い合わせください](https://forwardemail.net/help)。
...
8. メッセージに `Feedback-Type` ヘッダー（[RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965) に基づく ARF 不正利用フィードバックレポートを示す）が含まれている場合、送信しません。

9. MAIL FROM アドレスが ARF フィードバック送信者のパターン（例: `feedback@arf.mail.yahoo.com`）と一致する場合、送信しません。
...
以下の手順に従って、いつでもアカウントを削除することができます。

1. [forwardemail.net/login](https://forwardemail.net/login) でアカウントにログインします
2. [My Account → Security](/my-account/security) に移動します
3. ページの一番下までスクロールし、**"Delete Account"** ボタンをクリックします
...
<div class="alert my-3 alert-warning">
  ...
  <span>
    アカウントの削除は永久的であり、元に戻すことはできません。すべてのドメイン、エイリアス、メールボックス、メール、および関連データは、当社のシステムから完全に削除されます。有料プランをご利用の場合、サブスクリプションは直ちにキャンセルされます。
  </span>
</div>
