...
우리는 160만 개 이상의 도메인에 이메일 호스팅 및 이메일 전달 서비스를 제공하며, 다음과 같은 주목할 만한 사용자가 있습니다:
...
* 유료 플랜(강화된 보호 및 팀)에서는 별칭별로 WKD 조회를 비활성화할 수 있습니다. 별칭 설정으로 이동하여 "WKD 조회 비활성화" 옵션을 선택하세요. 비활성화되면 해당 별칭으로 전달되는 수신 이메일은 수신자의 공개 키가 게시되어 있더라도 WKD를 통해 암호화되지 않습니다. 이것은 API에서 `has_wkd_disabled` 필드를 사용하여도 이용할 수 있습니다.
...
발신자 속도 제한은 발신자 IP 주소에 대한 역 PTR 조회에서 파싱된 루트 도메인별로 적용되거나, 결과가 없으면 단순히 발신자 IP 주소를 사용합니다. 아래에서는 이를 `Sender`라고 합니다.

우리 MX 서버는 [암호화된 IMAP 저장](/blog/docs/best-quantum-safe-encrypted-email-service)을 위해 수신된 메일에 대해 일일 제한을 둡니다:

* 개별 별칭(예: `you@yourdomain.com`) 기준으로 수신 메일을 속도 제한하는 대신, 별칭의 도메인 이름 자체(예: `yourdomain.com`)로 속도 제한을 합니다. 이는 `Senders`가 도메인 내 모든 별칭의 받은편지함을 한꺼번에 폭주시키는 것을 방지합니다.
* 속도 제한은 발신자 신뢰 수준에 기반한 계층형 시스템을 사용하여 적용됩니다:
  * **Tier 1 – 진실된 출처** (예: `gmail.com`, `microsoft.com`, `apple.com`): 전역적으로 하루 100 GB로 제한됩니다. 도메인별 및 버스트 제한에서 면제됩니다.
  * **Tier 2 – [허용 목록에 있는](#do-you-have-an-allowlist) 발신자**: 전역적으로 하루 10 GB로 제한됩니다. 도메인별 및 버스트 제한에서 면제됩니다.
  * **Tier 3 – 기타 모든 발신자**: 전역적으로 하루 1 GB 및/또는 1000 메시지로 제한되며, `Sender`+도메인별로 하루 1 GB 및/또는 1000 메시지, 그리고 `Sender`+도메인별로 분당 50 메시지의 버스트 제한이 적용됩니다.
* 버스트 제한은 고정 윈도우 카운터(60초)를 사용합니다. 윈도우는 첫 번째 메시지가 도착할 때 시작되며 후속 메시지와 관계없이 60초 후에 만료됩니다 — 각 메시지마다 슬라이드되거나 재설정되지 않습니다.
* 수신자 사서함당 일일 최대 100,000 메시지 제한이 있습니다. 이는 모든 계층에 적용되며 발신자 신뢰 수준과 관계없이 단일 사서함이 폭주하는 것을 방지합니다.

모든 속도 제한은 원자적으로 적용됩니다 — 카운터는 메시지가 저장되기 전에 증가하여 동시 요청이 제한을 우회할 수 있는 경쟁 조건을 제거합니다. 감소 작업(증가 후 저장 실패 시 사용)은 카운터가 음수가 되는 것을 방지하는 안전한 Lua 스크립트를 사용합니다.

MX 서버는 또한 속도 제한을 통해 하나 이상의 수신자에게 전달되는 메시지를 제한합니다 – 하지만 이는 [허용 목록](#do-you-have-an-allowlist)에 없는 `Senders`에게만 적용됩니다:

* 시간당 최대 100 연결만 허용하며, 이는 `Sender`의 확인된 FQDN 루트 도메인 (또는) `Sender` 원격 IP 주소(역 PTR이 없는 경우)별, 그리고 봉투 수신자별로 적용됩니다. 속도 제한 키는 Redis 데이터베이스에 암호화 해시로 저장됩니다.
...
* 속도 제한을 초과하는 `Senders`는 421 오류로 거부됩니다.
...
이러한 제한은 사용자 계정별로 적용되며(별칭이나 도메인별이 아님) 매일 초기화됩니다. 즉, 추가 별칭을 생성해도 대역폭 할당량이 증가하지 않습니다. Redis를 사용할 수 없는 경우 속도 제한이 완전히 건너뛰어져(fail-open) 서비스가 중단되지 않습니다.
