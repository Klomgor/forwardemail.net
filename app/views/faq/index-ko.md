...
* `발신자`와 `yourdomain.com`별로 분당 50개 메시지의 버스트 제한이 있습니다. 이는 일일 한도에 도달하지 않았더라도 스패머가 초당 수백 개의 메시지로 도메인을 범람시키는 것을 방지합니다.

모든 속도 제한은 원자적으로 적용됩니다 — 카운터는 메시지가 저장되기 전에 증가하여 동시 요청이 제한을 우회할 수 있는 경쟁 조건을 제거합니다.
