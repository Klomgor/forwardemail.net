...
我们为超过 160 万个域名提供邮件托管和邮件转发服务，以下是一些知名用户：
...
7. 如果邮件包含 `Feedback-Type` 标头（根据 [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965) 指示为 ARF 滥用反馈报告），我们将不会发送。

8. 如果 MAIL FROM 地址匹配 ARF 反馈发件人模式（例如 `feedback@arf.mail.yahoo.com`），我们将不会发送。
...
* 在我们的付费计划（增强保护和团队）中，您可以按别名禁用 WKD 查找。导航到您的别名设置并勾选"禁用 WKD 查找"选项。禁用后，转发到该别名的入站电子邮件将不会通过 WKD 加密，即使已为收件人发布了公钥。这也可以通过 API 使用 `has_wkd_disabled` 字段来实现。
...
* 我们不是基于单个别名（例如 `you@yourdomain.com`）来限制入站邮件速率，而是基于别名的域名本身（例如 `yourdomain.com`）进行限制。这防止了 `Senders` 一次性向你域名下所有别箱的收件箱发送大量邮件。
* 速率限制是基于发件人信任级别的分层系统应用的：
...
* 超过速率限制的 `Senders` 将被拒绝，返回 421 错误。
...
这些限制按别名计算，每天重置。如果Redis不可用，速率限制将完全跳过（fail-open），因此您的服务永远不会中断。

如果您需要更高的限制用于特定用例（例如迁移非常大的存档），请[联系我们](https://forwardemail.net/help)。
...
8. 如果邮件包含 `Feedback-Type` 标头（根据 [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965) 指示为 ARF 滥用反馈报告），我们将不会发送。

9. 如果 MAIL FROM 地址匹配 ARF 反馈发件人模式（例如 `feedback@arf.mail.yahoo.com`），我们将不会发送。
...
1. 在 [forwardemail.net/login](https://forwardemail.net/login) 登录您的账户
2. 导航至 [我的账户 → 安全](/my-account/security)
3. 滚动到页面底部，然后点击 **"删除账户"** 按钮
4. 在出现提示时确认删除
...
<div class="alert my-3 alert-warning">
  ...
  <span>
    账户删除是永久性的，无法撤销。您的所有域名、别名、邮箱、电子邮件及相关数据将从我们的系统中永久删除。如果您使用的是付费计划，您的订阅将立即被取消。
  </span>
</div>
