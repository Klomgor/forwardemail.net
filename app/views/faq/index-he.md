...
* [הגדרת דואר אלקטרוני](#email-configuration)
  ...
  * [מדוע דוחות ה-DMARC שלי מראים כשלים מכתובות IP לא ידועות](#why-do-my-dmarc-reports-show-failures-from-unknown-ip-addresses)
...
* [תכונות מתקדמות](#advanced-features)
  ...
  * [האם אני יכול להעביר דוא"ל עבור כל תת-דומיין (תתי-דומיינים מסוג wildcard)?](#can-i-forward-email-for-any-subdomain-wildcard-subdomains)
...
* [פרטים מערכתיים וטכניים](#system-and-technical-details)
  ...
  * [מהן מגבלות רוחב הפס שלכם](#what-are-your-bandwidth-limits)
...
7. איננו שולחים אם להודעה יש כותרת `Feedback-Type` (המציינת דוח משוב על שימוש לרעה מסוג ARF לפי [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965)).

8. איננו שולחים אם כתובת MAIL FROM תואמת תבנית שולח משוב של ARF (לדוגמה `feedback@arf.mail.yahoo.com`).
...
אם דוחות ה-DMARC שלך מכילים כתובות IP לא מוכרות שנכשלות ב-SPF וב-DKIM, **אנא אל תדאג** – זה נורמלי ולמעשה אומר שההגדרה שלך עובדת כראוי.

דוחות DMARC מפרטים את **כל** השרתים שניסו לשלוח דואר בטענה שהוא מהדומיין שלך, כולל שרתים לא מורשים. בכמעט כל המקרים, כתובות ה-IP הלא ידועות הללו הן פשוט שולחי ספאם או בוטים שמנסים להתחזות לדומיין שלך. הנה מה שקורה כשהם מנסים:

* **SPF נכשל** מכיוון שכתובת ה-IP השולחת אינה רשומה ברשומת ה-SPF שלך.
* **DKIM נכשל** מכיוון שהם אינם יכולים לייצר חתימה חוקית עבור הדומיין שלך.
* עם מדיניות DMARC של `p=reject`, ההודעה **נדחית על הסף** ולעולם אינה מגיעה לתיבת הדואר הנכנס של איש.

במילים אחרות, כשלים אלו הם הוכחה לכך ש-DMARC עושה את עבודתו – חסימת ניסיונות התחזות לפני שהם גורמים נזק. לא נדרשת כל פעולה מצידך.

**כיצד להבחין בין שולחים לגיטימיים למתחזים**

* דואר שאתה שולח דרך Forward Email (או כל שירות מורשה אחר) יראה ש-SPF ו/או DKIM **עוברים ותואמים**.
* מקורות לא ידועים שנכשלים **גם** ב-SPF וגם ב-DKIM הם מתחזים וניתן להתעלם מהם בבטחה.
* אתה יכול להצליב כתובות IP מול <https://forwardemail.net/ips> כדי לאשר אילו מקורות הם שלנו.

אם אתה משתמש גם בשירותים אחרים כדי לשלוח דואר בשם הדומיין שלך (כלי שיווק, מערכות CRM וכו'), ודא שלשירותים אלה מוגדרת חתימת DKIM חוקית – אחרת ההודעות שלהם יופיעו גם ככשלים בדוחות שלך.

> \[!IMPORTANT]
> ודא שהדומיין שלך מפרסם מדיניות DMARC של `p=reject` כדי שניסיונות התחזות יידחו במקום להימסר. ראה [כיצד להגדיר DMARC עבור Forward Email](#how-do-i-set-up-dmarc-for-forward-email).
...
* בתוכניות בתשלום שלנו (הגנה משופרת וצוות), ניתן להשבית חיפוש WKD על בסיס כינוי. נווט להגדרות הכינוי שלך וסמן את האפשרות "השבת חיפוש WKD". כאשר מושבת, הודעות דוא"ל נכנסות המועברות לכינוי זה לא יוצפנו באמצעות WKD גם אם פורסם מפתח ציבורי עבור הנמען. זה זמין גם דרך ה-API באמצעות השדה `has_wkd_disabled`.
...
כן, ב**תוכניות בתשלום** שלנו.  באפשרותך להגדיר דומיין שורש יחיד (למשל `example.com`) כך שתצורת ההעברה שלו תחול באופן שקוף על **כל** תת-דומיין (למשל `anything.example.com`, `mail.example.com`, `a.b.example.com`), מבלי ליצור תצורה נפרדת עבור כל תת-דומיין ומבלי להשתמש ברשומת DNS מסוג wildcard כגון `*.example.com`.

<div class="alert my-3 alert-warning">
  <i class="fa fa-exclamation-circle font-weight-bold"></i>
  <strong>תוכניות בתשלום בלבד (הצטרפות):</strong> תכונה זו זמינה בתוכניות בתשלום שלנו וכבויה כברירת מחדל.  עליך להפעיל אותה עבור הדומיין תחת <strong>החשבון שלי &rarr; דומיינים &rarr; הגדרות</strong> על ידי סימון <strong>"אפשר העברת תת-דומיין מסוג Wildcard"</strong>.  היא <strong>אינה</strong> חלה על התוכנית החינמית.
</div>

לאחר ההפעלה, כאשר דוא"ל מגיע לנמען בתת-דומיין, אנו מחפשים תחילה את רשומות ה-<strong class="notranslate">TXT</strong> במארח תת-הדומיין המדויק הזה.  אם לתת-הדומיין המדויק אין רשומות `forward-email-site-verification=` משלו, אנו חוזרים אוטומטית לרשומת האימות שפורסמה בדומיין השורש (כך שתת-הדומיין יורש את אותם כינויים ואימות כמו דומיין השורש).

זה מוגבל בכוונה כדי שהתצורה הקיימת שלך לא תשתנה לעולם:

* יש להפעיל זאת במפורש עבור כל דומיין, וזה חל רק על התוכניות בתשלום שלנו (זה לעולם לא בשימוש בתוכנית החינמית).
* זה חל רק על תתי-דומיינים (דומיין השורש/הקודקוד עצמו אינו מושפע).
* זה חל רק כאשר לתת-הדומיין המדויק **אין** רשומות רלוונטיות, כך שכל רשומה שתפרסם בתת-דומיין ספציפי תמיד תקבל עדיפות על פני החזרה לדומיין השורש.
* רק רשומות `forward-email=` ו-`forward-email-site-verification=` עוברות בירושה מדומיין השורש.

<div class="alert my-3 alert-secondary">
  <i class="fa fa-info-circle font-weight-bold"></i>
  <strong>דוגמה לתת-דומיין מסוג Wildcard:</strong> לאחר הפעלת <strong>"אפשר העברת תת-דומיין מסוג Wildcard"</strong> עבור `example.com`, דואר שנשלח לכל תת-דומיין שאין לו רשומות משלו (לדוגמה `hello@anything.example.com`) יורש את התצורה של דומיין השורש, כולל רשומת האימות שלו:
</div>
...
#### רשומות DNS נדרשות עבור תתי-דומיינים מסוג wildcard {#required-dns-records-for-wildcard-subdomains}

דוא"ל מנותב על ידי רשומות ה-<strong class="notranslate">MX</strong> של כל נמען, כך שכדי שדואר יגיע אלינו פיזית עבור **כל** תת-דומיין, עליך לפרסם רשומות <strong class="notranslate">MX</strong> המכסות את תתי-הדומיינים שלך.  הגישה הפשוטה ביותר היא רשומת **MX מסוג wildcard** יחידה (`*`) אצל ספק ה-DNS שלך, אשר חלה על כל תת-דומיין בבת אחת:
...
<table class="table table-striped table-hover my-3">
  <thead class="thead-dark">
    <tr>
      ...
      <th class="text-center">TTL</th>
      <th>סוג</th>
      <th class="text-center">עדיפות</th>
      <th>תשובה/ערך</th>
    </tr>
  </thead>
  <tbody>
...
Wildcard כגון `*.example.com` תואם ל-`mail.example.com`, `a.b.example.com`, וכן הלאה.  כדי לכסות רק תת-דומיין ספציפי אחד במקום זאת, השתמש בתת-דומיין זה בתור השם/מארח (לדוגמה `mail` עבור `mail.example.com`) עם אותם שני ערכי <strong class="notranslate">MX</strong> שלמעלה.

חלק מספקי ה-DNS תומכים גם ב-<strong class="notranslate">CNAME</strong> מסוג wildcard (לדוגמה `*.example.com CNAME example.com`) כך שתתי-דומיינים יפנו לדומיין השורש שלך.  <strong class="notranslate">MX</strong> מסוג wildcard מועדף עבור מסירת דואר.

<div class="alert my-3 alert-warning">
  <i class="fa fa-exclamation-circle font-weight-bold"></i>
  <strong>חשוב:</strong> אל תוסיף רשומת <strong class="notranslate">CNAME</strong> על השורש/קודקוד (`@`) עצמו, מכיוון שהיא מתנגשת עם רשומות ה-<strong class="notranslate">MX</strong>, ה-<strong class="notranslate">TXT</strong> ורשומות אחרות שלך.  השאר את רשומת ה-<strong class="notranslate">TXT</strong> של `forward-email-site-verification=` מפורסמת בדומיין השורש שלך &mdash; תתי-דומיינים יורשים אותה אוטומטית.
</div>

#### אסימוני החלפת תת-דומיין {#subdomain-substitution-tokens}

כאשר אתה משתמש ב<a href="#do-you-support-regular-expressions-or-regex" class="alert-link">ביטויים רגולריים</a> בנמען (החלפה), באפשרותך בנוסף להתייחס לתת-הדומיין של הנמען הנכנס באמצעות שני אסימונים.  שים לב שכדי שאסימונים אלה יחולו על **כל** תת-דומיין מרשומת דומיין שורש יחידה, יש להפעיל את החזרה לתת-דומיין מסוג wildcard שתוארה לעיל (תוכניות בתשלום בלבד); אחרת הם חלים רק על רשומות שפורסמו במארח המדויק שתואם:
...
<table class="table table-striped table-hover my-3">
  ...
  <tbody>
    <tr>
      ...
      <td>תווית(ות) תת-הדומיין מתחת לדומיין השורש של הנמען הנכנס.  לדוגמה, עבור `team@sales.example.com` (שורש `example.com`) זהו `sales`, ועבור `x@a.b.example.com` זהו `a.b`.  עבור דומיין השורש/קודקוד זוהי מחרוזת ריקה.</td>
    </tr>
    <tr>
      <td><code>%HOST%</code></td>
      <td>המארח המלא (דומיין) של הנמען הנכנס.  לדוגמה, עבור `team@sales.example.com` זהו `sales.example.com`.</td>
    </tr>
  </tbody>
</table>
...
<div class="alert my-3 alert-secondary">
  <i class="fa fa-info-circle font-weight-bold"></i>
  <strong>דוגמה להחלפת תת-דומיין:</strong> אם ברצונך שכל כתובת בכל תת-דומיין של `example.com` תועבר לספק יחיד תוך שמירה על תת-הדומיין ביעד (למשל `anyone@sales.example.com` &rarr; `sales@example.net` ו-`anyone@support.example.com` &rarr; `support@example.net`), פרסם רשומה יחידה בדומיין השורש:
</div>
...
* מגבלות הקצב מיושמות באמצעות מערכת מדורגת המבוססת על רמת האמון של השולח:
  * **Tier 1 – מקורות אמת** (למשל `gmail.com`, `microsoft.com`, `apple.com`): מוגבלים ל-100 גיגה-בייט ליום באופן גלובלי. פטורים ממגבלות פרץ ומגבלות לכל דומיין.
  * **Tier 2 – שולחים [ברשימת ההרשאה](#do-you-have-an-allowlist)**: מוגבלים ל-10 גיגה-בייט ליום באופן גלובלי. פטורים ממגבלות פרץ ומגבלות לכל דומיין.
  * **Tier 3 – כל שאר השולחים**: מוגבלים ל-1 גיגה-בייט ו/או 1000 הודעות ליום באופן גלובלי, 1 גיגה-בייט ו/או 1000 הודעות לכל `Sender`+דומיין ביום, ומגבלת פרץ של 50 הודעות לכל `Sender`+דומיין לדקה.
* מגבלת הפרץ משתמשת במונה חלון קבוע (60 שניות). החלון מתחיל כאשר ההודעה הראשונה מגיעה ופג תוקפו לאחר 60 שניות ללא קשר להודעות הבאות — הוא אינו מחליק או מתאפס בכל הודעה.
* יש לנו מגבלה יומית של 100,000 הודעות לכל תיבת דואר של נמען. זה חל על כל הדירוגים ומונע מכל תיבת דואר בודדת להיות מוצפת ללא קשר לרמת האמון של השולח.

כל מגבלות הקצב נאכפות באופן אטומי — המונים מוגדלים לפני אחסון ההודעה, מה שמבטל מצבי מרוץ שבהם בקשות מקבילות יכולות לעקוף את המגבלות. פעולות הפחתה (המשמשות כאשר האחסון נכשל לאחר ההגדלה) משתמשות בסקריפטים בטוחים של Lua המונעים מהמונים להפוך לשליליים.
...
אנו אוכפים מגבלות רוחב פס לכל משתמש בכל השירותים כדי למנוע התקפות הצפה תוך שמירה על נדיבות מספקת לשימוש לגיטימי.  מגבלות אלה גבוהות בכוונה הרבה מעל אלה של Gmail — תוכלו לייבא גיבויים גדולים, לסנכרן את כל תיבת הדואר שלכם למכשיר חדש, או להשתמש במספר לקוחות בו-זמנית ללא כל מכשול.

| מגבלה | היקף | כמות |
| :--- | :--- | :---: |
| סה"כ יומי | כל השירותים יחד (IMAP, POP3, SMTP, CalDAV, CardDAV) | **50 GB** |
| שעתי לכל שירות | לכל שירות בנפרד (למשל הורדת IMAP, העלאת SMTP) | **10 GB** |

המגבלה היומית היא תקציב משותף אחד בין כל הפרוטוקולים — בין אם אתם מורידים דרך IMAP, מעלים דרך SMTP, או מסנכרנים לוחות שנה דרך CalDAV, הכל נספר לאותם 50 GB/יום.  המגבלה השעתית לכל שירות היא רשת ביטחון נגד סקריפטים שיצאו משליטה או חשבונות שנפרצו על פרוטוקול בודד — לא משהו שמשתמש לגיטימי צריך אי פעם להגיע אליו.

מגבלות אלה הן לכל חשבון משתמש (לא לכל כינוי או דומיין) ומתאפסות יומית.  משמעות הדבר היא שיצירת כינויים נוספים אינה מגדילה את הקצאת רוחב הפס שלכם.  אם Redis אינו זמין, הגבלת הקצב מדולגת לחלוטין (fail-open) כך שהשירות שלכם לעולם לא יופרע.

אם אתם צריכים מגבלות גבוהות יותר למקרה שימוש ספציפי (למשל, העברת ארכיון גדול מאוד), אנא [צרו קשר](https://forwardemail.net/help).
...
8. איננו שולחים אם להודעה יש כותרת `Feedback-Type` (המציינת דוח משוב על שימוש לרעה מסוג ARF לפי [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965)).

9. איננו שולחים אם כתובת MAIL FROM תואמת תבנית שולח משוב של ARF (לדוגמה `feedback@arf.mail.yahoo.com`).
