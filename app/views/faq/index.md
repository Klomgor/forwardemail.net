...
* [Email Configuration](#email-configuration)
  ...
  * [Why do my DMARC reports show failures from unknown IP addresses](#why-do-my-dmarc-reports-show-failures-from-unknown-ip-addresses)
...
* Mail you send through Forward Email (or any other authorized service) will show SPF and/or DKIM as **passing and aligned**.
* Unknown sources that fail **both** SPF and DKIM are impersonators and can be safely ignored.
* You can cross-reference IP addresses against <https://forwardemail.net/ips> to confirm which sources are ours.

If you also use other services to send mail on your domain's behalf (marketing tools, CRMs, etc.), make sure those services have a valid DKIM signature configured – otherwise their messages will also appear as failures in your reports.

> \[!IMPORTANT]
> Make sure your domain publishes a DMARC policy of `p=reject` so that impersonation attempts are rejected rather than delivered.  See [How do I set up DMARC for Forward Email](#how-do-i-set-up-dmarc-for-forward-email).
