...
* Limity rychlosti jsou aplikovány pomocí víceúrovňového systému založeného na úrovni důvěryhodnosti odesílatele:
  * **Úroveň 1 – Zdroje pravdy** (např. `gmail.com`, `microsoft.com`, `apple.com`): omezeno na 100 GB za den globálně. Vyjmuto z limitů pro doménu a nárazových limitů.
  * **Úroveň 2 – [Povolení](#do-you-have-an-allowlist) odesílatelé**: omezeno na 10 GB za den globálně. Vyjmuto z limitů pro doménu a nárazových limitů.
  * **Úroveň 3 – Všichni ostatní odesílatelé**: omezeno na 1 GB a/nebo 1000 zpráv za den globálně, 1 GB a/nebo 1000 zpráv na `Sender`+doménu denně, a limit pro nárazové odesílání 50 zpráv na `Sender`+doménu za minutu.
* Nárazový limit používá počítadlo s pevným oknem (60 sekund). Okno začíná, když dorazí první zpráva, a vyprší po 60 sekundách bez ohledu na následné zprávy — neposouvá se ani se neresetuje při každé zprávě.
* Máme denní limit na schránku příjemce ve výši 100,000 zpráv. To platí pro všechny úrovně a zabraňuje zahlcení jakékoli jednotlivé schránky bez ohledu na úroveň důvěryhodnosti odesílatele.

Všechny limity jsou vynucovány atomicky — počítadla se zvyšují před uložením zprávy, čímž se eliminují závodní podmínky, kdy by souběžné požadavky mohly obejít limity. Operace snížení (používané, když uložení selže po zvýšení) používají bezpečné Lua skripty, které zabraňují tomu, aby počítadla klesla do záporných hodnot.
