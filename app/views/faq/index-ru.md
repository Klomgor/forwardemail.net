...
* [Конфигурация почты](#email-configuration)
  ...
  * [Почему в моих отчетах DMARC отображаются ошибки с неизвестных IP-адресов](#why-do-my-dmarc-reports-show-failures-from-unknown-ip-addresses)
...
* [Расширенные функции](#advanced-features)
  ...
  * [Могу ли я пересылать электронную почту для любого поддомена (поддомены с подстановочными знаками)](#can-i-forward-email-for-any-subdomain-wildcard-subdomains)
...
* [Системные и технические детали](#system-and-technical-details)
  ...
  * [Каковы ваши лимиты пропускной способности](#what-are-your-bandwidth-limits)
...
* [Аккаунт и оплата](#account-and-billing)
  * [Как удалить свой аккаунт](#how-do-i-delete-my-account)
...
7. Мы не отправляем, если сообщение содержит заголовок `Feedback-Type` (указывающий на отчет о нарушениях ARF согласно [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965)).

8. Мы не отправляем, если адрес MAIL FROM соответствует шаблону отправителя отчетов ARF (например, `feedback@arf.mail.yahoo.com`).
...
Если в ваших отчетах DMARC содержатся незнакомые IP-адреса, не прошедшие проверку SPF и DKIM, **пожалуйста, не волнуйтесь** – это нормально и на самом деле означает, что ваша настройка работает правильно.

В отчетах DMARC перечисляется **каждый** сервер, который пытался отправить письмо от имени вашего домена, включая неавторизованные. Почти во всех случаях эти неизвестные IP-адреса — это просто спамеры или боты, пытающиеся выдать себя за ваш домен. Вот что происходит, когда они пытаются это сделать:

* **Сбой SPF**, потому что отправляющий IP-адрес не указан в вашей записи SPF.
* **Сбой DKIM**, потому что они не могут создать действительную подпись для вашего домена.
* При политике DMARC `p=reject` сообщение **отклоняется сразу** и никогда не попадает ни в чей почтовый ящик.

Другими словами, эти ошибки являются доказательством того, что DMARC выполняет свою работу – блокирует попытки подмены до того, как они причинят вред. От вас не требуется никаких действий.

**Как отличить легитимных отправителей от злоумышленников**

* Письма, которые вы отправляете через Forward Email (или любой другой авторизованный сервис), будут показывать SPF и/или DKIM как **успешные и согласованные**.
* Неизвестные источники, которые не проходят проверку **и** SPF, **и** DKIM, являются злоумышленниками, и их можно смело игнорировать.
* Вы можете сверить IP-адреса с <https://forwardemail.net/ips>, чтобы подтвердить, какие источники являются нашими.

Если вы также используете другие сервисы для отправки писем от имени вашего домена (маркетинговые инструменты, CRM и т. д.), убедитесь, что в этих сервисах настроена действительная подпись DKIM – иначе их сообщения также будут отображаться как ошибки в ваших отчетах.

> \[!IMPORTANT]
> Убедитесь, что ваш домен публикует политику DMARC `p=reject`, чтобы попытки подмены отклонялись, а не доставлялись. См. [Как настроить DMARC для Forward Email](#how-do-i-set-up-dmarc-for-forward-email).
...
* В наших платных планах (Усиленная защита и Команда) вы можете отключить поиск WKD для каждого псевдонима отдельно. Перейдите в настройки псевдонима и отметьте опцию "Отключить поиск WKD". При отключении входящие письма, пересылаемые на этот псевдоним, не будут шифроваться через WKD, даже если для получателя опубликован открытый ключ. Это также доступно через API с помощью поля `has_wkd_disabled`.
...
```
**Тихое удаление нежелательных сообщений:**
...
Да, на наших **платных тарифах**.  Вы можете настроить один корневой домен (например, `example.com`) так, чтобы его конфигурация пересылки прозрачно применялась к **каждому** поддомену (например, `anything.example.com`, `mail.example.com`, `a.b.example.com`), без создания отдельной конфигурации для каждого поддомена и без использования DNS-записи с подстановочным знаком, такой как `*.example.com`.

<div class="alert my-3 alert-warning">
  <i class="fa fa-exclamation-circle font-weight-bold"></i>
  <strong>Только для платных тарифов (по желанию):</strong> Эта функция доступна на наших платных тарифах и по умолчанию отключена.  Вы должны включить ее для домена в разделе <strong>Мой аккаунт &rarr; Домены &rarr; Настройки</strong>, установив флажок <strong>"Разрешить пересылку для поддоменов с подстановочными знаками"</strong>.  Это <strong>не</strong> относится к бесплатному тарифу.
</div>

После включения, когда электронное письмо поступает получателю на поддомене, мы сначала ищем записи <strong class="notranslate">TXT</strong> на этом конкретном хосте поддомена.  Если у конкретного поддомена нет собственных записей `forward-email-site-verification=`, то мы автоматически возвращаемся к записи верификации, опубликованной на корневом домене (таким образом, поддомен наследует те же псевдонимы и верификацию, что и корневой домен).

Это сделано намеренно узконаправленным, чтобы ваша существующая конфигурация никогда не изменялась:

* Это должно быть явно включено для каждого домена и применяется только к нашим платным тарифам (это никогда не используется на бесплатном тарифе).
* Это применяется только к поддоменам (сам корневой/apex домен не затрагивается).
* Это применяется только тогда, когда у конкретного поддомена **нет** соответствующих записей, поэтому любые записи, которые вы публикуете на определенном поддомене, всегда имеют приоритет над резервным вариантом корневого домена.
* От корневого домена наследуются только записи `forward-email=` и `forward-email-site-verification=`.

<div class="alert my-3 alert-secondary">
  <i class="fa fa-info-circle font-weight-bold"></i>
  <strong>Пример поддомена с подстановочным знаком:</strong> После включения <strong>"Разрешить пересылку для поддоменов с подстановочными знаками"</strong> для `example.com`, почта, отправленная на любой поддомен, у которого нет собственных записей (например, `hello@anything.example.com`), наследует конфигурацию корневого домена, включая его запись верификации:
</div>
...
#### Необходимые DNS-записи для поддоменов с подстановочными знаками {#required-dns-records-for-wildcard-subdomains}

Электронная почта маршрутизируется с помощью записей <strong class="notranslate">MX</strong> каждого получателя, поэтому для того, чтобы почта физически доходила до нас для **любого** поддомена, вы должны опубликовать записи <strong class="notranslate">MX</strong>, которые охватывают ваши поддомены.  Самый простой подход — это одна запись **MX с подстановочным знаком** (`*`) у вашего DNS-провайдера, которая применяется ко всем поддоменам одновременно:
...
Подстановочный знак, такой как `*.example.com`, соответствует `mail.example.com`, `a.b.example.com` и так далее.  Чтобы охватить только один конкретный поддомен, используйте этот поддомен в качестве Имени/Хоста (например, `mail` для `mail.example.com`) с теми же двумя значениями <strong class="notranslate">MX</strong>, указанными выше.

Некоторые DNS-провайдеры также поддерживают <strong class="notranslate">CNAME</strong> с подстановочным знаком (например, `*.example.com CNAME example.com`), чтобы поддомены разрешались в ваш корневой домен.  Для доставки почты предпочтительнее использовать <strong class="notranslate">MX</strong> с подстановочным знаком.

<div class="alert my-3 alert-warning">
  <i class="fa fa-exclamation-circle font-weight-bold"></i>
  <strong>Важно:</strong> Не добавляйте запись <strong class="notranslate">CNAME</strong> на самом корневом/apex домене (`@`), так как это конфликтует с вашими записями <strong class="notranslate">MX</strong>, <strong class="notranslate">TXT</strong> и другими.  Оставьте запись <strong class="notranslate">TXT</strong> `forward-email-site-verification=` опубликованной на вашем корневом домене &mdash; поддомены унаследуют ее автоматически.
</div>

#### Токены подстановки поддомена {#subdomain-substitution-tokens}

Когда вы используете <a href="#do-you-support-regular-expressions-or-regex" class="alert-link">регулярные выражения</a> в получателе (подстановке), вы можете дополнительно ссылаться на поддомен входящего получателя, используя два токена.  Обратите внимание, что для того, чтобы эти токены применялись к **каждому** поддомену из одной записи корневого домена, должен быть включен описанный выше резервный вариант поддомена с подстановочным знаком (только для платных тарифов); в противном случае они применяются только к записям, опубликованным на точно совпадающем хосте:
...
<table class="table table-striped table-hover my-3">
  ...
  <tbody>
    <tr>
      ...
      <td>Метка(и) поддомена ниже корневого домена входящего получателя.  Например, для `team@sales.example.com` (корень `example.com`) это `sales`, а для `x@a.b.example.com` это `a.b`.  Для корневого/apex домена это пустая строка.</td>
    </tr>
    <tr>
      <td><code>%HOST%</code></td>
      <td>Полный хост (домен) входящего получателя.  Например, для `team@sales.example.com` это `sales.example.com`.</td>
    </tr>
  </tbody>
</table>
...
<div class="alert my-3 alert-secondary">
  <i class="fa fa-info-circle font-weight-bold"></i>
  <strong>Пример подстановки поддомена:</strong> Если вы хотите, чтобы каждый адрес на каждом поддомене `example.com` пересылался одному провайдеру с сохранением поддомена в месте назначения (например, `anyone@sales.example.com` &rarr; `sales@example.net` и `anyone@support.example.com` &rarr; `support@example.net`), опубликуйте одну запись на корневом домене:
</div>
...
* Лимиты скорости применяются с использованием многоуровневой системы, основанной на уровне доверия к отправителю:
  * **Уровень 1 – Доверенные источники** (например, `gmail.com`, `microsoft.com`, `apple.com`): ограничены отправкой 100 ГБ в день глобально. Освобождены от лимитов на домен и лимитов на всплески.
  * **Уровень 2 – Отправители в [белом списке](#do-you-have-an-allowlist)**: ограничены отправкой 10 ГБ в день глобально. Освобождены от лимитов на домен и лимитов на всплески.
  * **Уровень 3 – Все остальные отправители**: ограничены отправкой 1 ГБ и/или 1000 сообщений в день глобально, 1 ГБ и/или 1000 сообщений на `Отправителя`+домен ежедневно, и лимитом на всплески в 50 сообщений на `Отправителя`+домен в минуту.
* Лимит на всплески использует счетчик с фиксированным окном (60 секунд). Окно начинается, когда поступает первое сообщение, и истекает через 60 секунд независимо от последующих сообщений — оно не сдвигается и не сбрасывается при каждом сообщении.
* У нас есть ежедневный лимит на почтовый ящик получателя в размере 100,000 сообщений. Это применяется ко всем уровням и предотвращает затопление любого отдельного почтового ящика независимо от уровня доверия к отправителю.

Все лимиты скорости применяются атомарно — счётчики увеличиваются до сохранения сообщения, что устраняет состояния гонки, при которых одновременные запросы могли бы обойти лимиты. Операции уменьшения (используемые, когда сохранение не удается после увеличения) используют безопасные Lua-скрипты, которые предотвращают уход счетчиков в отрицательные значения.
...
Мы применяем лимиты пропускной способности для каждого пользователя во всех сервисах для предотвращения атак флуда, оставаясь при этом достаточно щедрыми для легитимного использования.  Эти лимиты намеренно значительно превышают лимиты Gmail — вы можете импортировать большие резервные копии, синхронизировать весь почтовый ящик на новое устройство или использовать несколько клиентов одновременно без каких-либо ограничений.

| Лимит | Область | Объём |
| :--- | :--- | :---: |
| Суточный итого | Все сервисы вместе (IMAP, POP3, SMTP, CalDAV, CardDAV) | **50 ГБ** |
| Часовой на сервис | На отдельный сервис (напр. загрузка IMAP, выгрузка SMTP) | **10 ГБ** |

Суточный лимит — это единый общий бюджет для всех протоколов — загружаете ли вы через IMAP, выгружаете через SMTP или синхронизируете календари через CalDAV, всё считается в одни и те же 50 ГБ/день.  Часовой лимит на сервис — это страховочная сеть против неконтролируемых скриптов или скомпрометированных аккаунтов на одном протоколе — не то, чего легитимный пользователь когда-либо должен достичь.

Эти лимиты применяются к каждому аккаунту пользователя (не к псевдониму или домену) и сбрасываются ежедневно.  Это означает, что создание дополнительных псевдонимов не увеличивает вашу квоту пропускной способности.  Если Redis недоступен, ограничение скорости полностью пропускается (fail-open), чтобы ваш сервис никогда не прерывался.

Если вам нужны более высокие лимиты для конкретного случая использования (например, миграция очень большого архива), пожалуйста, [свяжитесь с нами](https://forwardemail.net/help).
...
8. Мы не отправляем, если сообщение содержит заголовок `Feedback-Type` (указывающий на отчет о нарушениях ARF согласно [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965)).

9. Мы не отправляем, если адрес MAIL FROM соответствует шаблону отправителя отчетов ARF (например, `feedback@arf.mail.yahoo.com`).
...
Вы можете удалить свой аккаунт в любое время, выполнив следующие действия:

1. Войдите в свой аккаунт на [forwardemail.net/login](https://forwardemail.net/login)
2. Перейдите в раздел [Мой аккаунт → Безопасность](/my-account/security)
3. Прокрутите страницу вниз и нажмите кнопку **"Удалить аккаунт"**
...
<div class="alert my-3 alert-warning">
  ...
  <span>
    Удаление аккаунта является необратимым и не может быть отменено. Все ваши домены, псевдонимы, почтовые ящики, электронные письма и связанные с ними данные будут навсегда удалены из наших систем. Если вы используете платный тарифный план, ваша подписка будет немедленно отменена.
  </span>
</div>

Если вы не можете войти в свой аккаунт, пожалуйста, свяжитесь с нами по адресу <a href="mailto:support@forwardemail.net">support@forwardemail.net</a> с адреса электронной почты, связанного с вашим аккаунтом, и мы поможем вам с удалением.
