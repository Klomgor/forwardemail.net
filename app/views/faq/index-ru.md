...
* У нас есть лимит на всплески — 50 сообщений на `Отправителя` и `yourdomain.com` в минуту. Это предотвращает затопление домена спамерами сотнями сообщений в секунду, даже если дневной лимит не достигнут.

Все лимиты скорости применяются атомарно — счётчики увеличиваются до сохранения сообщения, что устраняет состояния гонки, при которых одновременные запросы могли бы обойти лимиты.
