...
* [Конфігурація пошти](#email-configuration)
  ...
  * [Чому мої звіти DMARC показують помилки від невідомих IP-адрес](#why-do-my-dmarc-reports-show-failures-from-unknown-ip-addresses)
...
* [Розширені функції](#advanced-features)
  ...
  * [Чи можу я пересилати електронну пошту для будь-якого субдомену (субдомени з символами узагальнення)](#can-i-forward-email-for-any-subdomain-wildcard-subdomains)
...
* [Системні та технічні деталі](#system-and-technical-details)
  ...
  * [Які ваші обмеження пропускної здатності](#what-are-your-bandwidth-limits)
...
* [Обліковий запис та оплата](#account-and-billing)
  * [Як видалити свій обліковий запис](#how-do-i-delete-my-account)
...
> \[!TIP]
> Дізнайтеся більше про Forward Email, прочитавши наш безкоштовний [Технічний білет](/technical-whitepaper.pdf) та ознайомившись з нашим [аудитом безпеки](/pentest-report_forward-email.pdf) від Cure53.
...
7. Ми не надсилаємо, якщо повідомлення містить заголовок `Feedback-Type` (що вказує на звіт про зловживання ARF згідно з [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965)).

8. Ми не надсилаємо, якщо адреса MAIL FROM відповідає шаблону відправника відгуків ARF (наприклад, `feedback@arf.mail.yahoo.com`).
...
Якщо ваші звіти DMARC містять незнайомі IP-адреси, які не проходять перевірку SPF та DKIM, **будь ласка, не хвилюйтеся** – це нормально і насправді означає, що ваші налаштування працюють правильно.

У звітах DMARC перелічено **кожен** сервер, який намагався надіслати лист від імені вашого домену, включаючи неавторизовані. Майже у всіх випадках ці невідомі IP-адреси є просто спамерами або ботами, які намагаються видати себе за ваш домен. Ось що відбувається, коли вони намагаються це зробити:

* **SPF не проходить перевірку**, оскільки IP-адреса відправника не вказана у вашому записі SPF.
* **DKIM не проходить перевірку**, оскільки вони не можуть створити дійсний підпис для вашого домену.
* З політикою DMARC `p=reject`, повідомлення **відхиляється відразу** і ніколи не потрапляє до чиєїсь поштової скриньки.

Іншими словами, ці помилки є доказом того, що DMARC виконує свою роботу – блокує спроби підміни до того, як вони завдадуть шкоди. Від вас не вимагається жодних дій.

**Як відрізнити законних відправників від самозванців**

* Листи, які ви надсилаєте через Forward Email (або будь-який інший авторизований сервіс), показуватимуть SPF та/або DKIM як **успішні та узгоджені**.
* Невідомі джерела, які не проходять **обидві** перевірки SPF та DKIM, є самозванцями, і їх можна сміливо ігнорувати.
* Ви можете звірити IP-адреси з <https://forwardemail.net/ips>, щоб підтвердити, які джерела є нашими.

Якщо ви також використовуєте інші сервіси для надсилання листів від імені вашого домену (маркетингові інструменти, CRM тощо), переконайтеся, що ці сервіси мають налаштований дійсний підпис DKIM – інакше їхні повідомлення також відображатимуться як помилки у ваших звітах.

> \[!IMPORTANT]
> Переконайтеся, що ваш домен публікує політику DMARC `p=reject`, щоб спроби підміни відхилялися, а не доставлялися. Див. [Як налаштувати DMARC для Forward Email](#how-do-i-set-up-dmarc-for-forward-email).
...
* У наших платних планах (Посилений захист і Команда) ви можете вимкнути пошук WKD для кожного псевдоніма окремо. Перейдіть до налаштувань псевдоніма та позначте опцію "Вимкнути пошук WKD". Коли вимкнено, вхідні листи, що пересилаються на цей псевдонім, не будуть шифруватися через WKD, навіть якщо для одержувача опубліковано відкритий ключ. Це також доступно через API за допомогою поля `has_wkd_disabled`.
...
Так, на наших **платних тарифах**. Ви можете налаштувати єдиний кореневий домен (наприклад, `example.com`) так, щоб його конфігурація пересилання прозоро застосовувалася до **кожного** субдомену (наприклад, `anything.example.com`, `mail.example.com`, `a.b.example.com`), без створення окремої конфігурації для кожного субдомену та без використання запису DNS із символом узагальнення, такого як `*.example.com`.

<div class="alert my-3 alert-warning">
  <i class="fa fa-exclamation-circle font-weight-bold"></i>
  <strong>Тільки для платних тарифів (за бажанням):</strong> Ця функція доступна на наших платних тарифах і вимкнена за замовчуванням. Ви повинні ввімкнути її для домену в розділі <strong>Мій обліковий запис &rarr; Домени &rarr; Налаштування</strong>, поставивши прапорець <strong>"Дозволити пересилання для субдоменів із символами узагальнення"</strong>. Вона <strong>не</strong> застосовується до Безкоштовного тарифу.
</div>

Після ввімкнення, коли надходить електронний лист для одержувача на субдомені, ми спочатку шукаємо записи <strong class="notranslate">TXT</strong> на цьому точному хості субдомену. Якщо точний субдомен не має власних записів `forward-email-site-verification=`, ми автоматично повертаємося до запису перевірки, опублікованого на кореневому домені (тому субдомен успадковує ті самі псевдоніми та перевірку, що й кореневий домен).

Це навмисно обмежено, щоб ваша існуюча конфігурація ніколи не змінювалася:

* Це має бути явно ввімкнено для кожного домену і застосовується лише до наших платних тарифів (ніколи не використовується на Безкоштовному тарифі).
* Це застосовується лише до субдоменів (сам кореневий/вершинний домен залишається без змін).
* Це застосовується лише тоді, коли точний субдомен **не має** відповідних записів, тому будь-які записи, які ви публікуєте на певному субдомені, завжди мають пріоритет над резервним варіантом кореневого домену.
* Від кореневого домену успадковуються лише записи `forward-email=` та `forward-email-site-verification=`.

<div class="alert my-3 alert-secondary">
  <i class="fa fa-info-circle font-weight-bold"></i>
  <strong>Приклад субдомену із символом узагальнення:</strong> Після ввімкнення <strong>"Дозволити пересилання для субдоменів із символами узагальнення"</strong> для `example.com`, пошта, надіслана на будь-який субдомен, який не має власних записів (наприклад, `hello@anything.example.com`), успадковує конфігурацію кореневого домену, включаючи його запис перевірки:
</div>
...
#### Необхідні записи DNS для субдоменів із символами узагальнення {#required-dns-records-for-wildcard-subdomains}

Електронна пошта маршрутизується за допомогою записів <strong class="notranslate">MX</strong> кожного одержувача, тому, щоб пошта фізично досягла нас для **будь-якого** субдомену, ви повинні опублікувати записи <strong class="notranslate">MX</strong>, які охоплюють ваші субдомени. Найпростіший підхід — це єдиний запис **MX із символом узагальнення** (`*`) у вашого провайдера DNS, який застосовується до кожного субдомену одночасно:
...
<table class="table table-striped table-hover my-3">
  <thead class="thead-dark">
    <tr>
      ...
      <th class="text-center">TTL</th>
      <th>Тип</th>
      <th class="text-center">Пріоритет</th>
      <th>Відповідь/Значення</th>
    </tr>
  </thead>
  <tbody>
...
Символ узагальнення, такий як `*.example.com`, відповідає `mail.example.com`, `a.b.example.com` і так далі. Щоб охопити лише один конкретний субдомен, використовуйте цей субдомен як Ім'я/Хост (наприклад, `mail` для `mail.example.com`) з тими самими двома значеннями <strong class="notranslate">MX</strong>, що й вище.

Деякі провайдери DNS також підтримують <strong class="notranslate">CNAME</strong> із символом узагальнення (наприклад, `*.example.com CNAME example.com`), щоб субдомени перетворювалися на ваш кореневий домен. Для доставки пошти краще використовувати <strong class="notranslate">MX</strong> із символом узагальнення.

<div class="alert my-3 alert-warning">
  <i class="fa fa-exclamation-circle font-weight-bold"></i>
  <strong>Важливо:</strong> Не додавайте запис <strong class="notranslate">CNAME</strong> на самому кореневому/вершинному домені (`@`), оскільки він конфліктує з вашими записами <strong class="notranslate">MX</strong>, <strong class="notranslate">TXT</strong> та іншими записами. Зберігайте запис <strong class="notranslate">TXT</strong> `forward-email-site-verification=` опублікованим на вашому кореневому домені &mdash; субдомени успадковують його автоматично.
</div>

#### Токени підстановки субдоменів {#subdomain-substitution-tokens}

Коли ви використовуєте <a href="#do-you-support-regular-expressions-or-regex" class="alert-link">регулярні вирази</a> в одержувачі (підстановка), ви можете додатково посилатися на субдомен вхідного одержувача за допомогою двох токенів. Зверніть увагу, що для того, щоб ці токени застосовувалися до **кожного** субдомену із запису єдиного кореневого домену, резервний варіант субдомену із символом узагальнення, описаний вище, має бути ввімкнено (тільки для платних тарифів); інакше вони застосовуються лише до записів, опублікованих на точному хості, що збігається:
...
<table class="table table-striped table-hover my-3">
  ...
  <tbody>
    <tr>
      ...
      <td>Мітка(и) субдомену нижче кореневого домену вхідного одержувача. Наприклад, для `team@sales.example.com` (корінь `example.com`) це `sales`, а для `x@a.b.example.com` це `a.b`. Для кореневого/вершинного домену це порожній рядок.</td>
    </tr>
    <tr>
      <td><code>%HOST%</code></td>
      <td>Повний хост (домен) вхідного одержувача. Наприклад, для `team@sales.example.com` це `sales.example.com`.</td>
    </tr>
  </tbody>
</table>
...
<div class="alert my-3 alert-secondary">
  <i class="fa fa-info-circle font-weight-bold"></i>
  <strong>Приклад підстановки субдомену:</strong> Якщо ви хочете, щоб кожна адреса на кожному субдомені `example.com` пересилалася до одного провайдера зі збереженням субдомену в пункті призначення (наприклад, `anyone@sales.example.com` &rarr; `sales@example.net` та `anyone@support.example.com` &rarr; `support@example.net`), опублікуйте єдиний запис на кореневому домені:
</div>
...
* Обмеження швидкості застосовуються за допомогою багаторівневої системи на основі рівня довіри до відправника:
  * **Рівень 1 – Надійні джерела** (наприклад, `gmail.com`, `microsoft.com`, `apple.com`): обмежені надсиланням 100 ГБ на день глобально. Звільнені від лімітів на домен та лімітів на сплески.
  * **Рівень 2 – Відправники в [білому списку](#do-you-have-an-allowlist)**: обмежені надсиланням 10 ГБ на день глобально. Звільнені від лімітів на домен та лімітів на сплески.
  * **Рівень 3 – Всі інші відправники**: обмежені надсиланням 1 ГБ і/або 1000 повідомлень на день глобально, 1 ГБ і/або 1000 повідомлень на `Sender`+домен щодня, та лімітом на сплески у 50 повідомлень на `Sender`+домен на хвилину.
* Ліміт на сплески використовує лічильник з фіксованим вікном (60 секунд). Вікно починається, коли надходить перше повідомлення, і закінчується через 60 секунд незалежно від наступних повідомлень — воно не зсувається і не скидається з кожним повідомленням.
* Ми маємо щоденний ліміт на поштову скриньку отримувача у розмірі 100,000 повідомлень. Це застосовується до всіх рівнів і запобігає затопленню будь-якої окремої поштової скриньки незалежно від рівня довіри до відправника.

Усі ліміти швидкості застосовуються атомарно — лічильники збільшуються до збереження повідомлення, що усуває стани гонки, при яких одночасні запити могли б обійти ліміти. Операції зменшення (використовуються, коли збереження не вдається після збільшення) використовують безпечні Lua-скрипти, які запобігають тому, щоб лічильники ставали від'ємними.
...
Ми застосовуємо обмеження пропускної здатності для кожного користувача у всіх сервісах для запобігання атакам флуду, залишаючись при цьому достатньо щедрими для легітимного використання.  Ці обмеження навмисно значно перевищують обмеження Gmail — ви можете імпортувати великі резервні копії, синхронізувати всю поштову скриньку на новий пристрій або використовувати кілька клієнтів одночасно без будь-яких перешкод.

| Обмеження | Область | Обсяг |
| :--- | :--- | :---: |
| Добовий загалом | Усі сервіси разом (IMAP, POP3, SMTP, CalDAV, CardDAV) | **50 ГБ** |
| Погодинно на сервіс | На окремий сервіс (напр. завантаження IMAP, вивантаження SMTP) | **10 ГБ** |

Добовий ліміт — це єдиний спільний бюджет для всіх протоколів — чи завантажуєте ви через IMAP, вивантажуєте через SMTP, чи синхронізуєте календарі через CalDAV, все враховується в ті самі 50 ГБ/день.  Погодинний ліміт на сервіс — це страхувальна сітка проти некерованих скриптів або скомпрометованих облікових записів на одному протоколі — не те, чого легітимний користувач коли-небудь повинен досягти.

Ці обмеження застосовуються до кожного облікового запису користувача (не до псевдоніма чи домену) і скидаються щодня.  Це означає, що створення додаткових псевдонімів не збільшує вашу квоту пропускної здатності.  Якщо Redis недоступний, обмеження швидкості повністю пропускається (fail-open), щоб ваш сервіс ніколи не переривався.

Якщо вам потрібні вищі обмеження для конкретного випадку використання (наприклад, міграція дуже великого архіву), будь ласка, [зв'яжіться з нами](https://forwardemail.net/help).
...
8. Ми не надсилаємо, якщо повідомлення містить заголовок `Feedback-Type` (що вказує на звіт про зловживання ARF згідно з [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965)).

9. Ми не надсилаємо, якщо адреса MAIL FROM відповідає шаблону відправника відгуків ARF (наприклад, `feedback@arf.mail.yahoo.com`).
...
Ви можете видалити свій обліковий запис у будь-який час, виконавши такі дії:

1. Увійдіть у свій обліковий запис на [forwardemail.net/login](https://forwardemail.net/login)
2. Перейдіть до [Мій обліковий запис → Безпека](/my-account/security)
3. Прокрутіть сторінку вниз і натисніть кнопку **"Видалити обліковий запис"**
4. Підтвердьте видалення, коли з'явиться запит
...
<div class="alert my-3 alert-warning">
  ...
  <span>
    Видалення облікового запису є незворотним і не може бути скасовано.  Усі ваші домени, псевдоніми, поштові скриньки, електронні листи та пов’язані дані будуть назавжди видалені з наших систем.  Якщо ви користуєтеся платним планом, вашу підписку буде негайно скасовано.
  </span>
</div>

Якщо ви не можете увійти у свій обліковий запис, будь ласка, зв’яжіться з нами за адресою <a href="mailto:support@forwardemail.net">support@forwardemail.net</a> з електронної адреси, пов’язаної з вашим обліковим записом, і ми допоможемо вам із видаленням.
