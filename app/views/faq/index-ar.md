...
* [تكوين البريد الإلكتروني](#email-configuration)
  ...
  * [لماذا تظهر تقارير DMARC الخاصة بي حالات فشل من عناوين IP غير معروفة](#why-do-my-dmarc-reports-show-failures-from-unknown-ip-addresses)
...
* [تفاصيل النظام والتقنية](#system-and-technical-details)
  ...
  * [ما هي حدود عرض النطاق الترددي لديكم](#what-are-your-bandwidth-limits)
...
7. لا نرسل إذا كانت الرسالة تحتوي على ترويسة `Feedback-Type` (ما يشير إلى تقرير تغذية راجعة عن إساءة بتنسيق ARF وفقًا لـ [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965)).

8. لا نرسل إذا كان عنوان MAIL FROM يطابق نمط مُرسِل تغذية راجعة ARF (مثلًا `feedback@arf.mail.yahoo.com`).
...
إذا كانت تقارير DMARC الخاصة بك تحتوي على عناوين IP غير مألوفة تفشل في SPF و DKIM، **فلا تقلق من فضلك** – هذا أمر طبيعي ويعني في الواقع أن إعدادك يعمل بشكل صحيح.

تسرد تقارير DMARC **كل** خادم حاول إرسال بريد يدعي أنه من نطاقك، بما في ذلك الخوادم غير المصرح لها. في جميع الحالات تقريبًا، تكون عناوين IP غير المعروفة هذه مجرد مرسلي بريد عشوائي أو روبوتات تحاول انتحال شخصية نطاقك. إليك ما يحدث عندما يحاولون:

* **يفشل SPF** لأن عنوان IP المرسل غير مدرج في سجل SPF الخاص بك.
* **يفشل DKIM** لأنهم لا يستطيعون إنتاج توقيع صالح لنطاقك.
* مع سياسة DMARC الخاصة بـ `p=reject`، يتم **رفض الرسالة تمامًا** ولا تصل أبدًا إلى صندوق وارد أي شخص.

بعبارة أخرى، تعد حالات الفشل هذه دليلاً على أن DMARC يقوم بعمله – وهو حظر محاولات الانتحال قبل أن تتسبب في ضرر. لا يلزم اتخاذ أي إجراء من جانبك.

**كيفية التمييز بين المرسلين الشرعيين والمنتحلين**

* البريد الذي ترسله من خلال Forward Email (أو أي خدمة أخرى مصرح بها) سيُظهر SPF و/أو DKIM على أنهما **ناجحان ومتوافقان**.
* المصادر غير المعروفة التي تفشل في **كل من** SPF و DKIM هي منتحلة ويمكن تجاهلها بأمان.
* يمكنك التحقق من عناوين IP ومطابقتها مع <https://forwardemail.net/ips> لتأكيد أي المصادر تابعة لنا.

إذا كنت تستخدم أيضًا خدمات أخرى لإرسال البريد نيابة عن نطاقك (أدوات التسويق، CRM، إلخ)، فتأكد من تكوين توقيع DKIM صالح لتلك الخدمات – وإلا فستظهر رسائلها أيضًا كحالات فشل في تقاريرك.

> \[!IMPORTANT]
> تأكد من أن نطاقك ينشر سياسة DMARC الخاصة بـ `p=reject` بحيث يتم رفض محاولات الانتحال بدلاً من تسليمها. راجع [كيف أقوم بإعداد DMARC لـ Forward Email](#how-do-i-set-up-dmarc-for-forward-email).
...
* في خططنا المدفوعة (الحماية المحسنة والفريق)، يمكنك تعطيل بحث WKD على أساس كل لقب. انتقل إلى إعدادات اللقب الخاص بك وحدد خيار "تعطيل بحث WKD". عند التعطيل، لن يتم تشفير رسائل البريد الإلكتروني الواردة المعاد توجيهها إلى هذا اللقب عبر WKD حتى لو تم نشر مفتاح عام للمستلم. هذا متاح أيضًا عبر واجهة برمجة التطبيقات باستخدام حقل `has_wkd_disabled`.
...
* يتم تطبيق حدود المعدل باستخدام نظام متدرج يعتمد على مستوى ثقة المرسل:
  * **المستوى 1 – مصادر الحقيقة** (مثل `gmail.com`، `microsoft.com`، `apple.com`): محدود بـ 100 جيجابايت يوميًا عالميًا. معفى من حدود كل نطاق وحدود الرشقات.
  * **المستوى 2 – المرسلون [في القائمة المسموح بها](#do-you-have-an-allowlist)**: محدود بـ 10 جيجابايت يوميًا عالميًا. معفى من حدود كل نطاق وحدود الرشقات.
  * **المستوى 3 – جميع المرسلين الآخرين**: محدود بـ 1 جيجابايت و/أو 1000 رسالة يوميًا عالميًا، 1 جيجابايت و/أو 1000 رسالة لكل `Sender`+نطاق يوميًا، وحد للرشقات يبلغ 50 رسالة لكل `Sender`+نطاق في الدقيقة.
* يستخدم حد الرشقات عداد نافذة ثابتة (60 ثانية). تبدأ النافذة عند وصول الرسالة الأولى وتنتهي بعد 60 ثانية بغض النظر عن الرسائل اللاحقة — لا تنزلق أو يعاد تعيينها مع كل رسالة.
* لدينا حد أقصى يومي لصندوق بريد كل مستلم يبلغ 100,000 رسالة. ينطبق هذا على جميع المستويات ويمنع إغراق أي صندوق بريد فردي بغض النظر عن مستوى ثقة المرسل.

يتم تطبيق جميع حدود المعدل بشكل ذري — يتم زيادة العدادات قبل تخزين الرسالة، مما يزيل حالات السباق حيث يمكن للطلبات المتزامنة تجاوز الحدود. عمليات الإنقاص (المستخدمة عند فشل التخزين بعد الزيادة) تستخدم نصوص Lua آمنة تمنع العدادات من أن تصبح سلبية.
...
نحن نفرض حدود عرض نطاق ترددي لكل مستخدم عبر جميع الخدمات لمنع هجمات الإغراق مع البقاء سخيين بما يكفي للاستخدام المشروع.  هذه الحدود أعلى بكثير من حدود Gmail — يمكنك استيراد نسخ احتياطية كبيرة، ومزامنة صندوق بريدك بالكامل إلى جهاز جديد، أو استخدام عملاء متعددين في وقت واحد دون مواجهة أي عوائق.

| الحد | النطاق | المقدار |
| :--- | :--- | :---: |
| الإجمالي اليومي | جميع الخدمات مجتمعة (IMAP، POP3، SMTP، CalDAV، CardDAV) | **50 جيجابايت** |
| بالساعة لكل خدمة | لكل خدمة فردية (مثل تنزيل IMAP، رفع SMTP) | **10 جيجابايت** |

الحد اليومي هو ميزانية مشتركة واحدة عبر جميع البروتوكولات — سواء قمت بالتنزيل عبر IMAP، أو الرفع عبر SMTP، أو مزامنة التقويمات عبر CalDAV، فكل ذلك يُحسب ضمن نفس الـ 50 جيجابايت/يوم.  الحد بالساعة لكل خدمة هو شبكة أمان ضد البرامج النصية الخارجة عن السيطرة أو الحسابات المخترقة على بروتوكول واحد — وليس شيئاً يجب أن يصل إليه مستخدم شرعي.

هذه الحدود لكل حساب مستخدم (وليس لكل عنوان بريد أو نطاق) وتُعاد يومياً.  هذا يعني أن إنشاء عناوين بريد إضافية لا يزيد من حصة عرض النطاق الترددي الخاصة بك.  إذا كان Redis غير متاح، يتم تخطي تحديد المعدل بالكامل (فشل مفتوح) حتى لا تنقطع خدمتك أبداً.

إذا كنت بحاجة إلى حدود أعلى لحالة استخدام محددة (مثل ترحيل أرشيف كبير جداً)، يرجى [الاتصال بنا](https://forwardemail.net/help).
...
8. لا نرسل إذا كانت الرسالة تحتوي على ترويسة `Feedback-Type` (ما يشير إلى تقرير تغذية راجعة عن إساءة بتنسيق ARF وفقًا لـ [RFC 5965](https://datatracker.ietf.org/doc/html/rfc5965)).

9. لا نرسل إذا كان عنوان MAIL FROM يطابق نمط مُرسِل تغذية راجعة ARF (مثلًا `feedback@arf.mail.yahoo.com`).
